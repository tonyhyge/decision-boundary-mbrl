...
% Panel Dimensions & Grid Layout (Compact Gold Standard: 6.2cm Height)
\def\PW{5.95}  % Panel Width
\def\PH{6.2}   % Panel Height (~20% of IEEE journal page!)
...
\begin{scope}[shift={(0,0)}]
    ...
    \begin{scope}
        ...
        \draw[dangerred, dashed, line width=1.1pt] (0.3, 0.5) -- (5.6, 5.6);
        \draw[midgray, dashed, line width=0.75pt] (O) circle (1.45cm);
    \end{scope}

    \node[font=\tiny\bfseries, text=dangerred, rotate=43, anchor=south] at (4.35, 4.45) {Boundary $\Sigma: Q(a^*) = Q(a^{(2)})$};
...
% ==============================================================================
% PANEL (B): DECISION MARGIN GAUGE & 2x2 REGIME MATRIX
% ==============================================================================
\begin{scope}[shift={(\PW+\GAP, 0)}]
    ...
    % 2x2 Matrix of Regime Cards (Sleek & Ultra-Compact)
    \def\CW{2.6cm}
    
    % Regime 1 (Top Left)
    \node[boxcard, fill=safegreenbg, draw=safegreenborder, minimum width=\CW, text width=2.45cm, align=left] at (1.5, 3.65) {
    ...
    % Regime 2 (Top Right)
    \node[boxcard, fill=warnyellowbg, draw=warnyellowborder, minimum width=\CW, text width=2.45cm, align=left] at (4.45, 3.65) {
    ...
    % Regime 3 (Bottom Left)
    \node[boxcard, fill=tieorangebg, draw=tieorangeborder, minimum width=\CW, text width=2.45cm, align=left] at (1.5, 1.95) {
    ...
    % Regime 4 (Bottom Right)
    \node[boxcard, fill=dangerredbg, draw=dangerredborder, minimum width=\CW, text width=2.45cm, align=left] at (4.45, 1.95) {
    ...
    % Pressure Flow Arrows in 2x2 matrix
    \draw[->, line width=0.6pt, darkslate] (2.85, 3.65) -- (3.1, 3.65);
    \draw[->, line width=0.6pt, darkslate] (4.45, 2.85) -- (4.45, 2.75) -- (1.5, 2.75) -- (1.5, 2.65);
    \draw[->, line width=0.6pt, darkslate] (2.85, 1.95) -- (3.1, 1.95);
...
\begin{scope}[shift={(2*\PW+2*\GAP, 0)}]
    ...
    \node[font=\tiny\bfseries, text=backupcolor, anchor=north] at (\PW/2, \PH-0.55) {Global Backup: $V(s) \leftarrow \max_a [r + \gamma \mathbb{E} V(s')]$};

    % State Nodes with named handles
    \node[state] (N0) at (0.75, 4.35) {$s_0$};
    \node[state] (N1) at (2.15, 4.35) {$s_1$};
    \node[state] (N2) at (3.55, 4.35) {$s_2$};
    \node[state, draw=dangerred, fill=dangerredbg] (NC) at (4.95, 4.35) {\textcolor{dangerred}{$s_{\mathrm{cross}}$}};
    ...
    \draw[->, backupcolor, dashed, line width=1.0pt] 
        (NC.north) to[out=135, in=45] (N0.north);
    ...
    \node[boxcard, fill=dangerredbg, draw=dangerredborder, minimum width=\BW, text width=2.45cm, align=left, inner sep=2.5pt] 
        (B1) at (1.5, 2.7) {
            \textbf{\textcolor{dangerred}{1. Boundary Upweight}}\\
            \tiny $w(s_{\mathrm{cross}}) \uparrow$ $(\lambda > 0)$
        };

    \node[boxcard, fill=warnyellowbg, draw=warnyellowborder, minimum width=\BW, text width=2.45cm, align=left, inner sep=2.5pt] 
        (B2) at (4.45, 2.7) {
            \textbf{\textcolor{warnorange}{2. Corridor Depletion}}\\
            \tiny $w(s_1, s_2) \downarrow$ $(\sum w_i = N)$
        };

    \node[boxcard, fill=backupbg, draw=backupcolor!60, minimum width=\BW, text width=2.45cm, align=left, inner sep=2.5pt] 
        (B3) at (4.45, 1.5) {
